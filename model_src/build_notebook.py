"""Build notebooks/P2H2P_verification.ipynb -- the ENGINEERING record.

Code cells are EXTRACTED from /tmp/build/thums_model.py by function name, not
retyped, so the notebook runs the same code that the fixtures check. Markdown
cells carry the equations.

This notebook's job is to let a reader CHECK the model: fixtures, regression
against version control, the retired first-cycle sizing path kept for
comparison, and the reasoning behind every superseded approach. It shows every
line of code on purpose.

Someone who wants to USE the model should read P2H2P_model.ipynb instead, which
shows the equations and imports the implementation. Both are generated by
build_all.py from the same sources, so they cannot disagree.
"""
...
OUT = pathlib.Path('/tmp/build/P2H2P_verification.ipynb')
