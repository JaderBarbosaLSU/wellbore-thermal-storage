...
and the resistance that belongs with a mean is mean-to-surface. Treating the
control volume as an annulus $r_{e}\le r\le r_{o}$, adiabatic at $r_{o}$ by
symmetry with its neighbour and storing sensible heat at a uniform volumetric
rate, the quasi-steady balance $-k\,2\pi r\,\mathrm{d}T/\mathrm{d}r
=s\pi(r_{o}^{2}-r^{2})$ integrates and averages to
$R'_{\mathrm{bulk}}=S/2\pi k$ with
\begin{equation}
  S=\frac{\beta^{4}\ln\beta-\beta^{4}/2+\beta^{2}/2
    -\left(\beta^{2}-1\right)^{2}/4}{\left(\beta^{2}-1\right)^{2}},
  \qquad \beta=\frac{r_{o}}{r_{e}} .
  \label{eq:shape-factor}
\end{equation}
$S$ depends on geometry alone --- $k$ cancels --- so one shape factor serves both
phases. For the case studied $\beta=2.109$ and $S=0.3467$ against
$\ln\beta=0.7460$ for the surface-to-surface annulus: smaller by $2.152$, the
cylindrical counterpart of the familiar $1/3$ for a slab with uniform
generation. It enters the network as an equivalent thickness
$\delta_{\mathrm{eq}}=r_{e}(e^{S}-1)$, so the fin efficiency and finned-area
ratio apply unchanged.
