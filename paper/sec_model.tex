...
\subsection{Which shell the heat crosses}
\label{sec:shell}

Recovering a thickness from $\Amelt$ raises a question that the choice of state
variable makes unavoidable, and that an area-based formulation can leave
ambiguous. The PCM-side resistance is a shell growing \emph{outward from the
tube wall},
\begin{equation}
  R' = \frac{\ln\left(1+\delta/r_{e}\right)}{2\pi k},
  \label{eq:shell}
\end{equation}
so the model must state which material that shell is made of.

The front grows away from the tube in both directions, because the tube is the
driven boundary in both. While \emph{melting}, the shell is liquid and its
thickness follows $\Amelt$: heat crosses the melt to reach the remaining solid.
While \emph{freezing}, solidification also nucleates at the wall, so the shell
is the \emph{frozen} material, of thickness following $\Aav-\Amelt$, and the
liquid is displaced outward beyond the front where it takes no part in the
conduction path:
\begin{equation}
  \left(\delta,\,k\right) =
  \begin{cases}
    \left(\delta(\Amelt),\; k_{l}\right), & T_{j} > \Tpcm \quad\text{(melting)},\\[3pt]
    \left(\delta(\Aav-\Amelt),\; k_{s}\right), & T_{j} < \Tpcm \quad\text{(freezing)}.
  \end{cases}
  \label{eq:shell-choice}
\end{equation}
The direction is taken from the sign of the driving difference, which is known
before the conductance is formed and carries the sign of $q'$.

Equation~\eqref{eq:shell-choice} needs no special handling at the four limiting
states. A fully solid segment that is melting, and a fully melted segment that
is freezing, both have $\delta=0$ and therefore no PCM-side resistance --- the
front is at the tube and nothing has yet grown. A fully melted segment that is
melting, and a fully solid one that is freezing, both present the full annulus,
with $k_{l}$ and $k_{s}$ respectively.

Taking the melted thickness in both directions --- the arrangement one arrives at
by writing the melting case first and not revisiting it --- inverts the
freezing half-cycle, and the two thicknesses are complementary, so the error is
a \emph{reversal in time} rather than a scale factor. A segment frozen solid is
then assigned zero resistance where the physical resistance is greatest, and the
modelled exchanger improves as it freezes. In the case studied here that
produced an overall coefficient at the end of the discharge seven times its
value at the end of the charge, against $k_{s}/k_{l}=1.33$; with
Eq.~\eqref{eq:shell-choice} the two half-cycles become near mirror images,
agreeing to \SI{0.5}{\percent} at $t\to0$ and differing by $1.265$ at the end.

\paragraph{When there is no front.}
Equation~\eqref{eq:shell-choice} answers ``which side of the front'', which
presupposes one. A control volume that is entirely one phase --- subcooled solid
or superheated liquid --- has none, and the rule then returns a resistance that
depends on the direction of the heat flow. The same body of the same material
cannot have two resistances.

What changes at a branch boundary is the meaning of the state. In two-phase,
$\Tpcm=\Tm$ is the temperature of the \emph{front} and the appropriate
resistance is tube-to-front. In single phase $\Tpcm$ is the \emph{volume mean}
--- the state variable is the control volume's total energy, so the temperature
recovered from it can be nothing else --- and the resistance that belongs with a
mean is mean-to-surface, $R'_{\mathrm{bulk}}=(\overline{T}-T_{s})/Q'$.

Treat the control volume as an annulus $r_{e}\le r\le r_{o}$, adiabatic at
$r_{o}$ by symmetry with its neighbour, storing sensible heat at a uniform
volumetric rate $s$. Quasi-steady, whatever crosses radius $r$ is what the
material beyond it stores, $-k\,2\pi r\,\mathrm{d}T/\mathrm{d}r
=-s\pi(r_{o}^{2}-r^{2})$, which integrates to
\begin{equation}
  T(r)-T(r_{e}) = \frac{s}{2k}
...
$Q'=s\pi(r_{o}^{2}-r_{e}^{2})$ eliminates $s$ and leaves
$R'_{\mathrm{bulk}}=S/2\pi k$ with
\begin{equation}
  S=\frac{4\beta^{4}\ln\beta-3\beta^{4}+4\beta^{2}-1}
         {4\left(\beta^{2}-1\right)^{2}},
  \qquad \beta=\frac{r_{o}}{r_{e}} .
  \label{eq:shape-factor}
\end{equation}
$S$ depends on geometry alone --- $k$ cancels --- so one shape factor serves both
phases. Two checks fix it. Expanded for a thin annulus of wall thickness
$L=r_{o}-r_{e}$, Eq.~\eqref{eq:shape-factor} gives $S\to L/3r_{e}$ and hence
$R'_{\mathrm{bulk}}\to L/3kA$, the mean-to-surface resistance of a plane slab
with uniform generation; and for the case studied $\beta=2.109$ and $S=0.3467$
against $\ln\beta=0.7460$ for the surface-to-surface annulus, smaller by
$2.152$. It enters the network as an equivalent thickness
$\delta_{\mathrm{eq}}=r_{e}(e^{S}-1)=\SI{8.74}{\milli\meter}$, so the fin
efficiency and finned-area ratio apply unchanged.

The resistance is then discontinuous across a branch boundary, and that is not
an artefact: as $\varepsilon_{\mathrm{local}}\to1^{-}$ while melting the front
is at $r_{o}$ and $R'=\ln\beta/2\pi k_{l}$, while the instant the volume becomes
superheated $R'=S/2\pi k_{l}$. Nothing has moved; the reference temperature has
changed from the front to the mean, and the resistance follows it. The flux
remains bounded because Eq.~\eqref{eq:K} caps it at the capacity rate.

\paragraph{What a single thickness cannot do.}
One $\delta$ describes one front. After the first half-cycle from a uniformly
solid store the geometry is in general three-region: at cyclic steady state the
charge begins with residual liquid in the \emph{outer} part of the cell, so
melting produces liquid at the tube, solid in the middle and liquid outside.
Equation~\eqref{eq:shell-choice} is the better of the two single-front
approximations, not a resolution of that. It is the substance of hypothesis H5,
and it is the leading structural limitation of the treatment given here.
...
%=======================================================================
\section{Coupling to the power cycle: a pinch constraint, not an approach}
\label{sec:pinch}

The formulation above delivers heat as a \emph{glide}: the cascade is graded so
that the water sweeps the full $\Delta T_{3C,2C}$, and that glide is what pins
both mass flows. Any power cycle downstream must absorb it. This section states
the constraint that couples the two, because it is easy to state incorrectly and
the incorrect version is the natural one to write.

Let $T_{2d}$ and $T_{3d}$ be the hot and cold ends of the water on discharge. It
is tempting to fix the evaporating temperature of a downstream ORC by an
approach at the hot end,
\begin{equation}
  T_{3e} \;=\; T_{2d} - \Delta T_{2D,3E},
  \label{eq:hot-end}
\end{equation}
and to check nothing else. Equation~\eqref{eq:hot-end} is sufficient only when
the two streams have comparable heat-capacity curves. A subcritical ORC is the
case in which they do not: the working fluid absorbs the bulk of its duty
\emph{isothermally}, at $T_{3e}$, while the water is sensible throughout. A
horizontal composite and a sloping one meet somewhere in the interior, and the
binding condition is therefore
\begin{equation}
  \Delta T_{\mathrm{pinch}} \;=\;
  \min_{0 \le Q \le Q_{\mathrm{tot}}}
  \Bigl[\,T_{\mathrm{water}}(Q) - T_{\mathrm{wf}}(Q)\,\Bigr]
  \;\ge\; \Delta T_{\mathrm{pinch}}^{\min},
  \label{eq:pinch}
\end{equation}
evaluated over the whole exchanger. Both composites must be built on real
enthalpy: the water because $c_{p}$ varies appreciably over a glide of several
tens of kelvin, and the working fluid because the latent plateau is a vertical
segment in $(Q,T)$ and is destroyed by any linear-in-$T$ approximation.

The severity scales with the glide. Writing $\phi$ for the fraction of the
cycle's heat input absorbed at $T_{3e}$, the water can supply heat above $T_{3e}$
only over the portion of its own duty lying above that temperature. For a
constant-$c_{p}$ water stream this gives the necessary condition
\begin{equation}
  \frac{T_{2d}-T_{3e}}{T_{2d}-T_{3d}} \;\ge\; \phi ,
  \label{eq:pinch-necessary}
\end{equation}
so a cycle taking $\phi=0.76$ of its heat at one temperature requires
$T_{3e}$ to sit at least three quarters of the way down the glide --- which
Eq.~\eqref{eq:hot-end}, placing it a fixed small approach below the \emph{top},
violates by a margin that grows linearly with $\Delta T_{3C,2C}$.

The practical consequence for a storage concept of this kind is a design
tension that the sizing calculation does not otherwise expose. A wide glide is
attractive on the storage side: it grades the cascade, raises the PCM inventory
cycled per unit volume, and reduces the well count. The same width degrades the
power cycle, because Eq.~\eqref{eq:pinch-necessary} forces the evaporating
temperature down and the cycle efficiency with it. The optimum is interior, and
it cannot be found by treating $\Delta T_{3C,2C}$ as a free storage parameter.
Recovering the lost efficiency requires a working fluid that also glides --- a
zeotropic mixture --- or a supercritical or multi-pressure cycle; it does not
require a larger evaporator.

Section~\ref{sec:demo} reports the magnitude for the case studied: imposing
Eq.~\eqref{eq:pinch} in place of Eq.~\eqref{eq:hot-end} moves the evaporating
temperature from \SI{134.1}{\celsius} to \SI{95.4}{\celsius} and
$\eta_{\mathrm{ORC}}$ from $0.232$ to $0.174$.
