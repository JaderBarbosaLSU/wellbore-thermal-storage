...
driving temperature difference collapses, and the heat passes downstream. The
spread in local melt fraction along the exchanger falls from $1.321$ to
$0.078$, and the fraction of phase-change material that never cycles --- dead
...
\midrule
area-based & 1.78\; 0.73\; 0.69\; 0.71\; 0.77\; 0.85\; 0.94\; 1.03\; 1.12\; 1.23 & 1.321 \\
enthalpy   & 1.00\; 1.00\; 1.00\; 1.00\; 1.00\; 1.00\; 1.00\; 1.00\; 1.00\; 1.00
  & \textbf{0.078} \\
\bottomrule
\end{tabular}
\end{table}

Two distinct improvements appear in Table~\ref{tab:levelling}. Values above
unity are gone, because Eq.~\eqref{eq:amelt} makes them impossible. Values well
below unity are also gone, and this is the physical result: PCM that never melts
is dead volume, occupying the store and carrying no energy through the round
trip. The enthalpy formulation finds substantially less of it than the
area-based formulation implied. At the design point
$\varepsilon_{\mathrm{local}}$ spans $0.922$ to $1.000$ and the aggregate melted
fraction is $0.9992$.
...
\item Sensible capacity is self-levelling. Although it carries only about
  ...
  melting: the spread in local melted fraction falls from $1.321$ to $0.078$,
  and dead volume falls correspondingly. The effect is first-order in the
  distribution while being small in the energy, because it acts through the
  driving temperature difference rather than through the energy stored.
\end{enumerate}
...
of \S\ref{sec:demo}, which should accordingly be read as optimistic.
