...
that saturates goes on superheating past it, reaching \SI{15.6}{\kelvin} here,
so the measured figure is \SI{8.2}{\percent} rather than \SI{4.7}{\percent}
...
multiplies that cost by the number of cycles to convergence, of order \num{70}.
...
\node[blk, below=of in] (cyc) {%
  ...
  \begin{minipage}[t]{0.46\linewidth}
    ...
    $\Rightarrow\ \mathrm{COP}=2.887$
  \end{minipage}\hfill
  \begin{minipage}[t]{0.46\linewidth}
    \raggedright
    organic Rankine cycle\\ double-stage\\
    $\Rightarrow\ \eta_{\mathrm{ORC}}=0.1738$
  \end{minipage}\\[4pt]
...
\midrule
area-based (melted fraction capped) & 0.593 & 1.000 & 0.407 \\
...
$\varepsilon_{\mathrm{local}}=1$ --- \num{82} of the \num{100} segments, mean
$0.9706$ --- with a peak superheat of \SI{15.6}{\kelvin}, so a formulation whose
state stops at $\Tm$ has nowhere to put the last \SI{4.3}{\percent} of the
charge and nothing to draw the corresponding part of the discharge from.
...
\midrule
two-phase plateau, $0\le E'\le\Elat$ & 91.78 & --- \\
superheated liquid, $E'>\Elat$       &  4.31 & \SI{15.6}{\kelvin} \\
subcooled solid, $E'<0$              &  3.91 & \SI{17.1}{\kelvin} \\
\midrule
outside the plateau                  & \textbf{8.22} & \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
 9.000 & 0.7042 & 1.000 & 0.000 \\
12.000 & 0.9104 & 1.000 & 0.000 \\
14.000 & 1.0064 & 1.000 & 0.000 \\
15.659 & 1.0453 & 0.972 & 0.012 \\
17.000 & 1.0568 & 0.931 & 0.033 \\
18.000 & 1.0627 & 0.903 & 0.049 \\
\bottomrule
\end{tabular}
\end{table}
...
At the latent-only size the field delivers \SI{104.53}{\percent} of target. The
store comes close to melting completely at end of charge ---
$\varepsilon_{\mathrm{local}}=1$ at \num{82} of \num{100} stations, mean
$0.9715$ --- and returns to a mean of $0.0116$, so it very nearly refreezes. The
binding constraint at this design point is therefore closer to inventory than to
rate, and it is the design curve rather than any single number that separates
them. The
...
\begin{axis}[width=0.48\linewidth,height=5.4cm,
  ...
  title={\small (a) energy delivered}, ymin=0.66, ymax=1.10, xmin=8.5, xmax=18.5,
  grid=both, grid style={gray!20}]
\addplot[thick,mark=*,mark size=1.3pt,blue!70!black] coordinates {(9.00,0.70419) (12.00,0.91045) (14.00,1.00637) (15.66,1.04527) (17.00,1.05683) (18.00,1.06270)};
\addplot[dashed,gray,domain=8.5:16.5,samples=2,forget plot] {1};
\node[font=\scriptsize,gray] at (axis cs:16.6,0.965) {target};
\end{axis}
\begin{axis}[width=0.48\linewidth,height=5.4cm,at={(0.52\linewidth,0)},
  xlabel={$\Nwells$},
  ylabel={residual $\varepsilon_{\mathrm{local}}$},
  title={\small (b) unfrozen fraction at end of discharge},
  ymin=-0.003, ymax=0.055, xmin=8.5, xmax=18.5,
  grid=both, grid style={gray!20}]
\addplot[thick,mark=triangle*,mark size=1.6pt,orange!85!black] coordinates {(9.00,0.0000) (12.00,0.0000) (14.00,0.0000) (15.66,0.0116) (17.00,0.0326) (18.00,0.0491)};
\end{axis}
\end{tikzpicture}
...
$\Nwells\approx13.9$, \emph{below} the latent-only count of $15.66$.
...
\item Carrying enthalpy rather than melted area removes a structural defect in
  ...
  superheat of \SI{15.6}{\kelvin}, and \SI{8.2}{\percent} of the energy
  passing through the store each cycle traverses the sensible branches ---
  \SI{4.3}{\percent} as superheat and \SI{3.9}{\percent} as subcooling. That is
