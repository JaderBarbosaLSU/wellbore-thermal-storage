...
that saturates goes on superheating past it, reaching \SI{22.8}{\kelvin} here,
so the measured figure is \SI{10.3}{\percent} rather than \SI{4.7}{\percent}
...
Before the individual results, Figure~\ref{fig:procedure} places the formulation
in the calculation that uses it. The structure is worth stating plainly because
...
the organic Rankine cycle are evaluated once; they never see the store, and
their only outputs into the rest of the calculation are two efficiencies and the
five secondary-fluid state points that bracket each half-cycle. At the field
...
\node[inp] (in) {%
  ...
  \emph{approaches}\ \ $\Delta T_{4C,M}=\SI{10}{\kelvin}$,
...
\node[blk, below=of in] (cyc) {%
  ...
  \begin{minipage}[t]{0.46\linewidth}
    ...
    $\Rightarrow\ \eta_{\mathrm{ORC}}=0.2369$
  \end{minipage}\\[4pt]
  secondary-fluid state points\ \
  $T_{4c},T_{3c},T_{2c}$ (charge)\ \ and\ \ $T_{2d},T_{3d}$ (discharge)};
...
%------------------------------------------------------------------ march
\node[march, below=7mm of hw] (mar) {%
  \textbf{4.\ Half-cycle segment march} \hfill \emph{borehole level}\\[2pt]
  charge: inlet $T_{4c}$, flow $\md_{\mathrm{ch}}$\\[3pt]
...
\node[blk, below=of mar] (rev) {%
  ...
  $T_{3d}$ and flow $\md_{\mathrm{dc}}$. The two \SI{2}{\hour} rests are exact
  no-ops: the store is adiabatic and carries no axial conduction};
...
%------------------------------------------------------------------ outputs
\node[fin, below=of tst] (outp) {%
  \textbf{6.\ Results at cyclic steady state}\\[3pt]
...
%------------------------------------------------------------------ arrows
\foreach \a/\b in {in/cyc, cyc/bud, bud/hw, hw/mar, mar/rev, rev/tst}
  \draw[fl] (\a) -- (\b);
\draw[fl] (tst) -- node[lab, left] {yes} (outp);
...
\end{tikzpicture}
\caption{The solution procedure, as implemented. Blocks~1--3 are evaluated once
and produce a fully specified piece of hardware; blocks~4 and~5 are the segment
march of \S\ref{sec:formulation}--\S\ref{sec:integration}, executed twice per
cycle and repeated until the store reproduces its own initial state. The outer
loop is the only iteration: nothing here is obtained by root finding, which is a
consequence of Eq.~\eqref{eq:css-unity} rather than a choice
(\S\ref{sec:css}).}
\label{fig:procedure}
\end{figure}
...
$\varepsilon_{\mathrm{local}}=1$, with a peak superheat of \SI{22.8}{\kelvin},
so a formulation whose state stops at $\Tm$ has nowhere to put the last
\SI{8.6}{\percent} of the charge and nothing to draw the corresponding part of
the discharge from.
...
\midrule
two-phase plateau, $0\le E'\le\Elat$ & 89.70 & --- \\
superheated liquid, $E'>\Elat$       &  8.60 & \SI{22.8}{\kelvin} \\
subcooled solid, $E'<0$              &  1.71 & \SI{11.2}{\kelvin} \\
\midrule
outside the plateau                  & \textbf{10.30} & \\
\bottomrule
\end{tabular}
\end{table}
...
\item Carrying enthalpy rather than melted area removes a structural defect in
  ...
  superheat of \SI{22.8}{\kelvin}, and \SI{10.3}{\percent} of the energy
  passing through the store each cycle traverses the sensible branches ---
  \SI{8.6}{\percent} as superheat and \SI{1.7}{\percent} as subcooling. That is
