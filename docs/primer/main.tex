...
\begin{tabular}{@{}l r l@{}}
\toprule
$\Nwells$ & 11.626 & from latent heat alone, not solved \\
$\md^{\mathrm{ch}}$, $\md^{\mathrm{dc}}$ & 0.966, 0.971 &
  \si{\kilogram\per\second} per leg-pair \\
cycles to convergence & 73 & \\
$\eta_{\mathrm{storage}}$ & 1.000000 & Eq.~\eqref{eq:unity} \\
\midrule
\textbf{deviation} & \textbf{+4.731} & \si{\percent} \\
implied net output & 1.0473 & MWe against a target of 1.0000 \\
realised discharge glide & 57.56 & \si{\kelvin} against \num{55} assumed \\
realised outlet $T_{2d}$ & 148.7 & \si{\celsius} \\
\midrule
$\epsloc$ at end of charge & 0.9706 & \num{82} of \num{100} segments reach 1 \\
$\epsloc$ at end of discharge & 0.0094 & it very nearly refreezes \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
$\eta_T$ & discharge thermal $\to$ net electric & 0.1899 &
  the power block alone \\
$\eta_{T,\mathrm{eff}}$ & the same, less discharge pumping & 0.1851 &
  tube bore, flow, well depth \\
$\eta_{\mathrm{RTE}}$ & round trip, both parasitics & 0.4370 &
  COP, $\eta_{\mathrm{ORC}}$, parasitics \\
$\varepsilon_{\mathrm{RTE}}$ & $\eta_{\mathrm{RTE}}\times$ cycled fraction &
  0.4201 & anything leaving PCM unused \\
\midrule
$\Delta E_{\mathrm{therm}}$ & thermal energy per well & \SI{4744}{\kilo\watt\hour} &
  $\hm$, $\rho$, $V_{\mathrm{well}}$ \\
$\Delta E_{\mathrm{elec}}$ & net electric energy per well &
  \SI{878}{\kilo\watt\hour} & the above $\times\ \eta_{T,\mathrm{eff}}$ \\
$\rho_E$ & thermal energy density &
  \SI{171.4}{\kilo\watt\hour\per\meter\cubed} & $\hm$, $\rho$ \\
\midrule
cycled fraction & $\varepsilon_{\mathrm{ch}}-\varepsilon_{\mathrm{dc}}$ & 0.9612 &
  \\
pumping & charge / discharge & 26.0 / \SI{26.4}{\kilo\watt} &
  \SI{5.00}{\percent} of gross \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{And it very nearly empties.} The residual after discharge is $0.0094$.
