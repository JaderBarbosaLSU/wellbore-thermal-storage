...
\paragraph{Which shell the heat crosses --- and why it is not obvious.}
...
$$S=\frac{\beta^4\ln\beta-\beta^4/2+\beta^2/2-(\beta^2-1)^2/4}{(\beta^2-1)^2}.$$

\textbf{$S$ depends on geometry alone} --- $k$ cancels --- so one shape factor
does for both phases and only the conductivity changes. Here $\beta=2.109$ and
$S=0.3467$ against $\ln\beta=0.7460$ for the surface-to-surface annulus: smaller
by $2.152$, which is the cylindrical version of the $1/3$ you already know for a
slab with uniform generation.
...
That takes about \num{70} cycles here, and the count grows with the exchanger count.
...
\midrule
$\eta_T$ & discharge thermal $\to$ net electric & 0.1898 &
  the power block alone \\
$\eta_{T,\mathrm{eff}}$ & the same, less discharge pumping & 0.1850 &
  tube bore, flow, well depth \\
$\eta_{\mathrm{RTE}}$ & round trip, both parasitics & 0.4367 &
  COP, $\eta_{\mathrm{ORC}}$, parasitics \\
$\varepsilon_{\mathrm{RTE}}$ & $\eta_{\mathrm{RTE}}\times$ cycled fraction &
  0.4192 & anything leaving PCM unused \\
\midrule
...
$\Delta E_{\mathrm{elec}}$ & net electric energy per well &
  \SI{876}{\kilo\watt\hour} & the above $\times\ \eta_{T,\mathrm{eff}}$ \\
